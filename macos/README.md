...
```bash
swift run --package-path macos sharepaste-desktop
```
...
```bash
macos/.build/release/sharepaste-desktop
```

## Notes

- The app currently bridges to the existing `client` CLI (`npm run -w client dev -- ...`) for business operations.
